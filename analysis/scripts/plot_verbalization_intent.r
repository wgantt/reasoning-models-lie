...
data_long <- data %>%
  ...
  mutate(
    score_type = case_when(
      score_type == "faithfulness_score_intent_normalized" ~ "Faithfulness",
      score_type == "honesty_score_intent_normalized" ~ "Honesty",
      TRUE ~ score_type
    ),
...
data_long <- data_long %>%
  mutate(
    ci_lower = ifelse(score_type == "Faithfulness", 
                      faithfulness_score_intent_normalized_ci_lower, 
                      honesty_score_intent_normalized_ci_lower),
    ci_upper = ifelse(score_type == "Faithfulness", 
                      faithfulness_score_intent_normalized_ci_upper, 
                      honesty_score_intent_normalized_ci_upper),
...
# Save the plot
ggsave(
  "analysis/plots/verbalization_intent_barplot.png",
  plot = p,
  width = 20,
  height = 8,
  dpi = 300
