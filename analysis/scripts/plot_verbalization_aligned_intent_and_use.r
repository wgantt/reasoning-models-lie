...
data <- data %>%
  mutate(
    ...
    scores_match = faithfulness_score_normalized == honesty_score_aligned_intent_and_use_normalized
  )

# Reshape data to long format for both scores
data_long <- data %>%
  pivot_longer(
    cols = c(faithfulness_score_normalized, honesty_score_aligned_intent_and_use_normalized),
    names_to = "score_type",
  ...
  mutate(
    score_type = case_when(
      ...
      score_type == "honesty_score_aligned_intent_and_use_normalized" ~ "Honesty",
      TRUE ~ score_type
    ),
...
# Save the plot
ggsave(
  "analysis/plots/verbalization_use_and_intent_barplot.png",
  plot = p,
  width = 20,
  height = 8,
  dpi = 300
